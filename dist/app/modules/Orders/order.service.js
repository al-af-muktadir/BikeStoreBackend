...
const CreateOrderInDb = (orders, client_ip) => __awaiter(void 0, void 0, void 0, function* () {
    const { products, User } = orders;
    // //console.log('pds', products);
    if (!products || products.length === 0) {
        throw new AppError_1.AppError(http_status_codes_1.StatusCodes.NOT_FOUND, 'No Order Yet');
        // //console.log('sdada');
    }
    let totalPrice = 0;
    //console.log('tt', totalPrice);
    const productDetails = yield Promise.all(products === null || products === void 0 ? void 0 : products.map((item) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (const item of products) {
        ...
        // //console.log(bike);
        if (!bike) {
            throw new AppError_1.AppError(http_status_codes_1.StatusCodes.NOT_FOUND, 'Not Found');
        }
        // Check if requested quantity is available
        if (quantity > bike.quantity) {
            throw new AppError_1.AppError(http_status_codes_1.StatusCodes.NOT_FOUND, 'inSufficient stock');
...
const getEmailOrderFromDb = (email) => __awaiter(void 0, void 0, void 0, function* () {
    const user = yield user_model_1.userModel.findOne({ email: email });
    const result = yield order_model_1.default.find({ User: new mongodb_1.ObjectId(user === null || user === void 0 ? void 0 : user._id) });
    return result;
});
